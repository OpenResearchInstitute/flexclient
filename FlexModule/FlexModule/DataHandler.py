...
def ParseResponse(radio, string):
	...
	if "slice c" in sent_msg:
		if hex_code != 0:
			...
			newSlice = Slice(radio, float(slice_data["freq"]), slice_data["ant"], slice_data["mode"])
...
def ParsePanadapterPacket(packet, x, y, mindb, maxdb):
	...
	StartBin_index, NumBins, BinSize, TotalBinsInFrame, FrameIndex, PacketCount = unpack(">HHHHLL", packet.payload[index:index+16])
